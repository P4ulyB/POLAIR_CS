...
void APACS_SelectionCueProxy::BeginPlay()
{
	...
	// Snap to floor and update visuals
	SnapToFloor();
	UpdateVisualForViewer();
}

void APACS_SelectionCueProxy::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
static uint16 GetLocalViewerId(const APACS_SelectionCueProxy* Self)
{
	if (const UWorld* W = Self->GetWorld())
	{
		if (const APlayerController* PC = W->GetFirstPlayerController())
		{
			...
			{   // Use your short id mapping; fallback to lower 16 bits
...
{
	if (bIsLocallyHovered != bHovered) { bIsLocallyHovered = bHovered; UpdateVisualForViewer(); }
}
...
const FSelectionVisualSet& APACS_SelectionCueProxy::GetVisualSet() const
{
	...
	static FSelectionVisualSet Default; // zero-initialised
	return Default;
}
